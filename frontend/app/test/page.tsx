...
import { Home, ShoppingCart, Minus, Plus, Loader, ArrowUp } from "lucide-react";
import OrderNav from "@/components/OrderNav";
import { formatPrice } from "../lib/utils";
import {PreOrderInterface} from "@/types/PreOrderInterface"
import {PreOrderItemInterface} from "@/types/PreOrderItemInterface"
import {PreOrderWarehouseInterface} from "@/types/PreOrderWarehouseInterface";
// ===== МОКОВЫЕ ДАННЫЕ (полностью соответствуют интерфейсу PreOrderInterface) =====
const mockWarehouses: PreOrderWarehouseInterface[] = [
  ...
  { id: 5, address: "г. Сочи, ул. Навагинская, д. 8 (Южный регион)", isAvailableForOrder: false },
  { id: 6, address: "г. Екатеринбург, ул. Вайнера, д. 12 (Урал)", isAvailableForOrder: false },
];

const mockItems: PreOrderItemInterface[] = [
  {
    productId: 1,
    variantId: 101,
    brand: "SAINTS KELLY",
  ...
  },
  {
    ...
    isAvailable: true,
    price: { base: 185000, applied: 185000, type: "RETAIL" },
    subtotal: 185000,
    availableAtWarehouses: [
      { warehouseId: 1, availableQuantity: 2 },
    ],
  },
];
...
export default function OrderPage() {
  ...
  // ЗАКОММЕНТИРОВАНО: реальная авторизация
  ...
  // ===== ЗАГРУЗКА МОКОВ (вместо реального API) =====
  useEffect(() => {
    // ЗАКОММЕНТИРОВАНА проверка авторизации
    // if (!loginValue) {
    //   router.push("/");
    //   return;
    // }
    ...
    setTimeout(() => {
      ...
      const allIds = mockPreviewData.items.map((item) => item.variantId);
      ...
      // Автовыбор склада
      const availableWarehouses = mockPreviewData.warehouses.filter((w) => w.isAvailableForOrder);
      for (const wh of availableWarehouses) {
        const allAvailable = mockPreviewData.items.every((item) => {
          const stock = item.availableAtWarehouses.find((s) => s.warehouseId === wh.id);
      ...
      try {
        ...
        if (variantsParam && quantitiesParam) {
          ...
          const cartData = await new Promise<CartInterface | null>((resolve) => {
            getCart(createApiArgs({ setData: resolve, setError: () => {}, setLoading: () => {} }));
          });
        ...
        const success = await getPreview({
          ...
          ...createApiArgs({
            setData: (data) => {
              ...
              setQuantities(initialQuantities);
              const availableWarehouses = preview.warehouses.filter((w) => w.isAvailableForOrder);
              for (const wh of availableWarehouses) {
  ...
  */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Имитация отправки
    setTimeout(() => {
      console.log("ТЕСТОВЫЙ ЗАКАЗ ОФОРМЛЕН:", {
        items: orderItems,
      ...
      // router.push(`/orders/123`); // закомментировано для теста
    }, 1000);
  };
  ...
  return (
    <div className="h-screen flex flex-col max-w-[1680px] mx-auto">
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        {/* Навигация - заглушка (можно заменить на ваш компонент OrderNav) */}
        <OrderNav/>
